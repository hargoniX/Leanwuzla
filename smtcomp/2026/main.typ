...
#show: ieee.with(
  title: [`bv_decide` at the SMT-COMP 2026],
  abstract: [
    This paper presents our SMT solver `bv_decide` as submitted to SMT-COMP 2026.
...
`bv_decide` competes in the single-query track of the `QF_BV` logic at SMT-COMP 2026,
using CaDiCaL 2.1.2 @cadical as its SAT solver. We submit `bv_decide` in two configurations:
- *With kernel checking enabled:* fully verified, with end-to-end correctness guarantees.
- *With kernel checking disabled:* significantly faster, but only partially verified.

Disabling kernel checking improves performance substantially, but at the cost of expanding the
trusted code base to include Lean's rewrite engine and sacrificing full end-to-end certification.
Nevertheless, the configuration remains far more trustworthy than most solvers: the rewriting rules,
bit-blasting engine, and LRAT certificate validation are still formally verified.

= Improvements
- Improved rewriting engine
- Reduced memory consumption during UNSAT proof checking
